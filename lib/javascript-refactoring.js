...
module.exports = JavascriptRefactoring = {
  ...
  activate: function(/* state */) {
    ...
    var addConsoleLogLineNumberOnSelection_command = atom.commands.add('atom-workspace', {
      'javascript-refactoring:addConsoleLogLineNumberOnSelection': function() {
        return this.addConsoleLogLineNumberOnSelection();
      }.bind(this)
    });
    this.subscriptions.add(addConsoleLogLineNumberOnSelection_command);
  ...
  // each line from selection will receive a number
  // FIXME: convert this to a smart block AST manipulation
  addConsoleLogLineNumberOnSelection: function () {
    var editor = atom.workspace.getActiveTextEditor();

    // copy selection to memory
    var selected_text = editor.getSelectedText();

    // if there is not selection, go away
    if (!selected_text || selected_text.length === 0) {
      return;
    }
    ...
    for (var i = 0; i < selected_text_array.length; i++) {
      var line = selected_text_array[i];
      new_text_array.push('/**/console.log(\'>> ' + i + '\');' + "\/" + "*-inserted-debug-*\/");
      new_text_array.push(line);
    }

    editor.insertText(new_text_array.join('\n'));
  },
